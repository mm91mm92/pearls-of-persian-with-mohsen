...
| `/copyright/` | Contains copyright notice for different aspects of the website. |
| `/license/` | Contains all licenses terms applied tp different resources of the website. |
...
### 3.1 Breadcrumbs
Breadcrumbs are displayed at the top of many every pages (excluding the Home page, license, copyright), positioned below the site header and above the page title. They are usually constructed dynamically from the directory hierarchy using `meta.yml:title` definitions.
...
**Rules:**
* The root segment (`/`) always maps to the site title defined in the root `meta.yml:title`.
...
On directory index pages and literary document pages, a collapsible, responsive sidebar displays the canonical structure of the repository and the location of the current page within it.
...
* Clicking a directory node (poets, works, and structural divisions) navigates to its index page; clicking a literary/narrative unit node navigates to its literary document page.

### 3.3 Next / Previous Navigation
For literary document pages nested within a work directory, sequential navigation controls are rendered at the bottom of the page to allow readers to traverse the work in its canonical sequence as appears in a printed book.

**Traversing Logic:**
* **Standard Traversal:** "Next" navigates to the next sibling in the parent directory’s `meta.yml:children` array.
* **Structural Boundaries:** If the current document is the last child in its section's `children` array, "Next" must navigate to the first document of the subsequent section folder (obeying the order of sections defined in the book's `meta.yml:children` array). This recursive traversal logic applies across arbitrary nested directory depths.
...
**Rules:**
* H1 headings are ignored by the Table of Contents.
...
* If there is no H2 headings, then there is no need to show an empty TOC.
...
* A link to the copyright page which further links to the license page.:
	```html
	<a href="/copyright/">© 2026 mm91mm92 and contributors</a>
	```
...
### 4.3 The Copyright Page 
- **Source:** `COPYRIGHT.md` 
- **URL:** `/copyright/` 
- **Layout:** Uses `<RootLayout>`. The content of the Markdown file appears in the `<MiddleContainer>` converted into HTML. There are links to the license page in the file.
	```text
	┌────────────────────┐
	│    SiteHeader      │
	├────────────────────┤
...
### 4.4 The License Page
- **Source:** `LICENSE`.
- **URL:** `/license/`.
- **Layout:** Uses `<RootLayout>`. The Markdown content appears in the `<MiddleContainer>` converted into HTML.
	```text
	┌────────────────────┐
	│    SiteHeader      │
	├────────────────────┤
...
*   **Source:** `contributors/`
*   **URL:** `/contributors/`
*   **Layout:** Uses `<SemiNavLayout>`. The `<Breadcrumbs>` displays: `گوهرهای پارسی با محسن ‹ مشارکت‌کنندگان`. The final segment (`مشارکت‌کنندگان`) is unlinked. The `<MainContainer>` renders the global alphabetical list of contributors after project maintainers at the top of the list.
	```text
	┌────────────────────────────┐
	│         SiteHeader         │
	├────────────────────────────┤
...
*   **Layout:** Uses `<SemiNavLayout>`. The `<Breadcrumbs>` displays: `گوهرهای پارسی با محسن ‹ مشارکت‌کنندگان ‹ <id>`, where the final segment is unlinked. The `<MainContainer>` is split into four distinct sub-containers:
    *   **Title Container:** Large Persian name and smaller Latin name.
    *   **Info Container:** Professional location, and Persian/English institutional affiliation, Persian biography text, accompanied by an optional English translation (collapsed by default).
    *   **Links Container:** Email address, website URL, standardized external profiles (GitHub, LinkedIn, ORCID, etc.) rendered from the `links` flat dictionary.
    *   **Contributions Container:** A role-articles, two-level hierarchy: for every role, there is a list of articles.
...
### 4.5 Poet Page
*   **Source:** `corpus/<poet>/meta.yml`
*   **URL:** `/عطار-نیشابوری/`
*   **Layout:** Uses `<FullNavLayout>`. The `<SidebarTree>` displays the poet's location within the `corpus/` hierarchy. The `<MainContainer>` consists of the following sub-containers, stacked vertically regardless of screen size:
...
### 4.6 Book Page
*   **Source:** `corpus/<poet>/<book>/meta.yml`
*   **URL:** `/عطار-نیشابوری/منطق-الطیر/`
*   **Layout:** Uses `<FullNavLayout>`. The `<SidebarTree>` represents the book's location within the literary hierarchy, and the `<MainContainer>` consists of the following stacked components:
    *   **Title:** The book title, parent poet name as subtitle, and the `description` metadata (if present).
    *   **TOC:** A hierarchical, variable-depth structural tree of links showing all available structural divisions alongside with the ultimate narrative units.
...
### 4.7 Structural Division Pages
*   **Source:** `corpus/<poet>/<book>/<structural_division>/../<structural_division>/meta.yml`
*   **URL:** `/عطار-نیشابوری/منطق-الطیر/مقدمه/`
*   **Layout:** Uses `<FullNavLayout>`. The `<SidebarTree>` displays the structural division's location within the literary hierarchy. There might be any number of levels of structural divisions, while some literary works do not have such divisions and their TOC is a flat list of narrative units. The `<MainContainer>` consists of the following stacked components:
    *   **Title:** The section title; a multiline subtitle of the poet name, and book title, the parent section(s) title (if any); and the `description` metadata as description.
    *   **TOC:** A list or hierarchy depending on whether this structural division has any structural division as its children:
	      * **List:** there is no further structural division and a list exposes the literary documents under it.
	      * **Hierarchy:** there is at least one more level of structural division, so a hierarchy exhibits them alongside the ultimate literary documents.
...
*   **Source:** A `.md` file containing front matter, an H1 heading, and content blocks interspersed within H2 headings.
*   **URL:** `/عطار-نیشابوری/منطق-الطیر/مقدمه/مجمع-مرغان/`
*   **Layout:** Uses `<FullNavLayout>`. The `<SidebarTree>` represents the document's location within the hierarchy. The `<MainContainer>` container is divided into the following sub-containers:
...
export interface Url {
  ...
  '404' | 'site' | 'corpus' getType();
}
```

* **`getType`:** Specifies the type of this URL: 
	* **`corpus`:** The URL refers to a corpus resource such as a poet, a literary work, a structural division, or a narrative unit.
	* **`site`:** The URL refers to a non-corpus resource such as search, copyright, license, the contributors directory, a contributor profile etc. URLs.
	* **`404`:** The URL is neither points to a corpus resource nor the website resource.
...
export type Toc {
  ...
  TocItem | null next(TocItem);
  TocItem | null prev(TocItem);
}
```

`next` and `prev` returns the next and previous TOC item (heading) which is typographically written in a book: 
* **Next heading:** the next sibling if it is not the last children, otherwise the first sibling of its nearest ancestor whose is not the last sibling of its parent structural division or literary work. If this TOC item and all its ancestors are the last child of their respective parents, there is no "next" TOC item and the method returns `null`.
...
export interface Contributor {
  ...
  bio?: string;
  bio_fa?: string;
  preferred_roles?: Record<string, Url[]>;
...
### 5.4. `ExcerptBlock`
Properties required to format the core text segment of a content block (the unit of annotation in the project).
...
Defines glossary mapping for annotated content blocks.
...
### 5.6. `ContentBlockProps`

Properties defining an individual numbered unit of annotation literary text and its corresponding collapsible explanations.
...
### 5.10. `MetadataProps`
Properties required to display a document's historical, cryptographic, and scholarly metadata. 
```typescript
...
Packs all the information about a poet from the literary hierarchy:
...
Packs all the information about a literary work from the literary hierarchy:
...
The most important property is `toc`, almost all the other properties is a simple reflect from the corresponding `meta.yml` file in the `corpus/`. `toc` is actually the filesystem under the book which is visible through `meta.yml:children`.
...
*   **Purpose:** Renders an individual annotated content section (`### <number>`).
...
context = {
  cb: ContentBlockProps;
}
#}

<div class="ppwm-content-block" id="block-{{ cb.number }}">
  {% if cb.glossary or cb.meaning or cb.notes or cb.references %}
...
*   **Purpose:** Renders the compulsory excerpt block (`متن`), dynamically determining prose vs. poetic structural alignment.
*   **Props:** `ExcerptBlock`
*   **Jinja2 / Astro Template:**
```jinja2
{#
...
*   **Props:** `MetadataProps`
* **`status` Display Guidelines:** The `status` value does affect a literary document's visibility in the presentation layer. All documents remain publicly visible, accompanied by their corresponding workflow state status and matching visual indicator:
...
### 6.10. Contributor Component (`<ControbutorBlock>`)

**Purpose:** Renders a contributor TypeScript object into the HTML.

```typescript
export interface ContributorBlockProps{
  controbutor: Contributor,
  hide_name: bool,
}
```

```markdown
I want to develop the contributor component as a division (`div`), divided into three sections, each representing a bunch of key-value pairs: 
- The first section contains information about the contributor himself/herself. Information items like `name`/`name_fa`, `location` (if presents), `affiliation`/`affiliation_fa` (if available), and `bio`/`bio_fa` (if provided). Moat of information in this section usually has two representation of Persian and English, we can specify whether to display name (`name` and `name_fa`) or not due to its use in the `TitleBlock`.
- The second section contains links information: `email`, `website` (if present), and the key-value pairs in `links` field like `github`, `linkedin`, `orcid`, or `telegram`. All this section is in English.
- The third section encompasses a role-articles hierarchy: for every role, there is a list of articles.
```

### 6.11 Layout & Header/Footer Components
